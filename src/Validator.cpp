...
double Validator::getAccuracy(const vector<int> &feature_subset, Classifier *classifier) // 1 3 7 -> ~89
{
    ...
    DataSet *row_trimmed_data = rowTrimDataSet(0, new_trim_data);
    ...
    for (int i = 0; i < new_trim_data->getInstances().size(); i++)
    {
        ...
        classifier->train(row_trimmed_data);
        int resultOfTest = classifier->test(new_trim_data->getInstances().at(i));
